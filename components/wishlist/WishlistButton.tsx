...
export default function WishlistButton({
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex items-center justify-center rounded-full border border-black/[0.08] bg-white/95 text-black shadow-[0_4px_14px_-8px_rgba(0,0,0,0.35)] backdrop-blur-sm transition-[transform,background-color,color] duration-200 hover:scale-[1.04] hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-600/40 focus-visible:ring-offset-2",
        variant === "card" && "size-8 sm:size-9",
        variant === "pdp" && "size-11 sm:size-12",
        active && "border-green-600/20 text-green-700",
        className
      )}
    >
      <Heart
